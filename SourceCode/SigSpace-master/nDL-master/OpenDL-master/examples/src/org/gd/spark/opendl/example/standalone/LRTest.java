...
public class LRTest {
    ...
    public static void main(String[] args) {
        try {
            int x_feature = 784;
            int y_feature = 10;
            List<SampleVector> samples = DataInput.readMnist("data/mnist_784_1000.txt", x_feature, y_feature);
            ...
            config.useCG = true;
            config.cgEpochStep = 100;
            config.cgTolerance = 0;
            config.cgMaxIterations = 30;
            config.maxEpochs = 100;
            config.nbrModelReplica = 4;
            config.minLoss = 0.01;
            config.useRegularization = true;
            config.printLoss = true;
